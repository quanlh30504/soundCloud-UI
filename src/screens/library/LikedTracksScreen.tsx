...
import { playlistApi, likedTracksApi } from '../../services/api';
...
export default function LikedTracksScreen() {
  ...
  const fetchLikedTracks = async () => {
    setLoading(true);
    try {
      const tracksResponse = await likedTracksApi.getLikedTracks(0, 100);
      setTracks(tracksResponse.data.content);
      setTotalTracks(tracksResponse.data.totalElements);
    } catch (error) {
      console.error('Error fetching liked tracks:', error);
      Alert.alert('Error', 'Failed to load liked tracks. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRemoveTrackFromPlaylist = async (spotifyId: string) => {
    
    setRemovingTrack(true);
    try {
      await likedTracksApi.deleteSongFromLikedTracks(spotifyId);

      setTracks(tracks.filter(track => track.spotifyId !== spotifyId));
      setTotalTracks(prevCount => prevCount - 1);
  ...
  const coverImageUrl = 'https://fakeimg.pl/200x200';
            ...
            { 
              ...
              onPress: () => {
                if (!removingTrack && selectedTrack.id) {
                  handleRemoveTrackFromPlaylist(selectedTrack.spotifyId);
                }
              },
